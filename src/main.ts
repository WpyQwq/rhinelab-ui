...
import { LocalArchivePanel } from "./archive-panel";
import {
  fetchStatus,
  formatBytes,
  loadArchiveIndex,
...
let localArchive: LocalArchivePanel;
...
$("#stage").innerHTML = `
  ...
  <nav class="system-nav" aria-label="系统导航">
    ...
    <button data-action="local-archive" aria-label="本地归档检索与归档操作" title="本地归档：检索 E:\归档 并执行归档操作">LOCAL ARCHIVE</button>
  ...
  <button id="skip" class="skip" data-action="skip" aria-label="跳过开场，直接进入档案" title="跳过开场"><span class="skip-glyph" aria-hidden="true">↗</span></button>
  <button id="exit" class="exit" data-action="exit" aria-label="退出系统" title="退出系统"><span class="exit-glyph" aria-hidden="true">⏻</span></button>
  <div id="shutdown" class="shutdown" role="dialog" aria-modal="true" aria-label="退出系统" hidden>
    <div class="shutdown-mark">${logo}</div>
    <div class="shutdown-word"><strong>SHUTDOWN</strong><strong class="shutdown-highlight" aria-hidden="true">SHUTDOWN</strong></div>
  </div>
...
// 开场不再有"点击进入"门槛：应用启动后自动播放（见 beginOpening）。
// 浏览器要求用户手势才允许启动音频设备，所以加载时先尝试解锁；
// 解锁失败则本次开场静音，并在首次真实交互时补上（armAudioUnlock），
// **不清空**用户的音效偏好。
const ENTRY_LOGO_HOLD = 1000;
const ENTRY_UNLOCK_TIMEOUT = 1500;
...
let columnMemory: number[] = archiveColumns.map((_, lane) => columnFiles(lane)[0] ?? 0);
...
fit();
// 刻度条固定窗口：一列没有条数上限，一条一个按钮会渲染几百上千个节点，
// 也会撑破竖条布局。窗口随选中位置滑动，语义与阵列的循环一致。
const TICK_WINDOW = 8;
let fileTicks: HTMLButtonElement[] = [];
let tickLane = -1;
function tickWindow(lane: number, position: number) {
  const members = columnFiles(lane);
  const len = members.length;
  const count = Math.min(TICK_WINDOW, len);
  if (!count) return [] as { index: number; position: number }[];
  // 列不超窗口时按自然顺序铺满（1,2,3…），与改动前完全一致——
  // 否则「以选中项居中」会把短列的顺序整体转位：高亮停在正中，标签在旁边轮转。
  const start = len <= TICK_WINDOW ? 0 : position - Math.floor(count / 2);
  const out: { index: number; position: number }[] = [];
  for (let k = 0; k < count; k += 1) {
    const pos = (((start + k) % len) + len) % len;
    out.push({ index: members[pos], position: pos });
  }
  return out;
}
function renderTicks(lane: number) {
  const count = Math.min(TICK_WINDOW, columnFiles(lane).length);
  $("#file-ticks").innerHTML = Array.from({ length: count }, () => "<button></button>").join("");
  fileTicks = [...$("#file-ticks").querySelectorAll<HTMLButtonElement>("button")];
  tickLane = lane;
}
renderTicks(fileLocation(selected).lane);
...
function updateSelection(navigation?: ArchiveNavigation) {
  ...
  if (tickLane !== lane) renderTicks(lane);
  const position = fileLocation(selected).row - 12;
  tickWindow(lane, position).forEach((tick, slot) => {
    const button = fileTicks[slot];
    if (!button) return;
    const record = records[tick.index];
    button.dataset.select = String(tick.index);
    button.setAttribute("aria-label", `选择档案 ${record.id} ${record.title}`);
    button.title = `${record.id} · ${record.title}`;
    button.classList.toggle("selected", tick.position === position);
    button.setAttribute("aria-pressed", String(tick.position === position));
  });
...
function renderDetail() {
  tabTransition.cancel();
  const r = records[detailIndex()];
  $("#object-id").textContent = "NO." + String(detailIndex() + 1).padStart(3, "0");
  ...
  <div class="detail-actions"><button class="solid-button" data-action="bookmark">${saved.has(r.id) ? "− REMOVE FROM SAVED" : "＋ SAVE ARCHIVE"}<span>${saved.has(r.id) ? "已收藏" : "收藏档案"}</span></button><button class="export-button" type="button" data-action="reveal-archive" aria-label="在资源管理器中打开 ${r.id} 的文件位置" title="在资源管理器中打开该档案在 E:\归档 中的位置">SHOW IN FOLDER<span>文件位置</span></button></div>
  <div class="detail-footnote"><span class="detail-path" title="归档内的相对路径">${escapeHtml(r.path ?? r.source)}</span><span>${String(detailIndex() + 1).padStart(3, "0")} / ${String(records.length).padStart(3, "0")}</span></div>`;
...
function overview() {
  return `<div class="panel-label">ABSTRACT / 摘要</div><p>${escapeHtml(records[detailIndex()].abstract)}</p>`;
}

// ---------- 详情页 ----------
// 原版行为：索引就是那四十条档案，没有"列外条目"，
// 详情页始终跟随阵列选中项。
...
// 在资源管理器中定位当前档案。路径来自 content/archives.json 的真实归档路径，
// 由本地归档服务的 /reveal 处理；服务只监听回环，因此需要它处于运行状态。
async function revealSelectedArchive() {
  const record = records[detailIndex()];
  if (!record.path) {
    notify("这条档案没有对应的本地路径");
    return;
  }
  const status = await fetchStatus();
  if (!status) {
    notify("需要本地归档服务：请运行 npm run dev:archive");
    return;
  }
  try {
    await revealEntry(record.path);
    notify(`已在资源管理器中定位：${record.title}`);
  } catch (error) {
    notify(`无法打开文件位置：${(error as Error).message}`);
  }
}
...
// ---------- 主检索里的本地归档命中 ----------
// 那 40 条精选档案是固定集合，归档里新增的文件永远不会出现在上面的列表里。
// 这里补一段来自全量索引的命中，让"检索档案"覆盖整个 E:\归档。
const LOCAL_HIT_LIMIT = 5;
let localIndex: ArchiveIndex | null = null;
let localHitsTimer: ReturnType<typeof setTimeout> | undefined;
let localHitsToken = 0;

function renderLocalHits() {
  const host = document.querySelector<HTMLElement>("#local-hits");
  if (!host) return;
  const query = searchQuery.trim();

  if (modal !== "search" || !query) {
    host.hidden = true;
    host.innerHTML = "";
    host.closest(".terminal-modal")?.classList.remove("has-local-hits");
    return;
  }

  host.hidden = false;
  host.closest(".terminal-modal")?.classList.add("has-local-hits");

  if (!localIndex) {
    host.innerHTML = `<div class="local-hits-head"><span>LOCAL ARCHIVE / 本地归档</span><span>索引不可用 · 运行 npm run dev:archive 后重试</span></div>`;
    return;
  }

  const { hits, total } = searchArchive(localIndex, query, LOCAL_HIT_LIMIT);
  if (!total) {
    host.innerHTML = `<div class="local-hits-head"><span>LOCAL ARCHIVE / 本地归档</span><span>无命中</span></div>`;
    return;
  }

  host.innerHTML =
    `<div class="local-hits-head"><span>LOCAL ARCHIVE / 本地归档</span><span>命中 ${total} 项${
      total > hits.length ? ` · 显示前 ${hits.length} 项` : ""
    }<button data-action="local-more" data-query="${escapeHtml(query)}">全部 →</button></span></div>` +
    hits.map((entry) => localHitMarkup(entry)).join("");
}

function localHitMarkup(entry: ArchiveEntry) {
  const cut = entry.path.lastIndexOf("/");
  const dir = cut < 0 ? "" : entry.path.slice(0, cut);
  const name = cut < 0 ? entry.path : entry.path.slice(cut + 1);
  return `<div class="local-hit"><span class="local-hit-kind">${
    entry.kind === "d" ? "DIR" : "FILE"
  }</span><span class="local-hit-name">${escapeHtml(name)}<small>${escapeHtml(dir)}</small></span><span class="local-hit-size">${
    entry.kind === "d" ? "—" : formatBytes(entry.size)
  }</span><span class="local-hit-actions"><button data-action="local-reveal" data-path="${escapeHtml(
    entry.path,
  )}">定位</button><button data-action="local-open" data-path="${escapeHtml(
    entry.path,
  )}">打开</button></span></div>`;
}

function scheduleLocalHits() {
  clearTimeout(localHitsTimer);
  const query = searchQuery.trim();
  if (modal !== "search" || !query) {
    renderLocalHits();
    return;
  }
  localHitsTimer = setTimeout(() => void refreshLocalHits(), 140);
}

async function refreshLocalHits() {
  const token = ++localHitsToken;
  const query = searchQuery.trim();
  if (modal !== "search" || !query) return;
  renderLocalHits(); // 先按现有索引出一版，避免输入时空白
  try {
    const { index } = await loadArchiveIndex({ maxAgeMs: 15_000 });
    if (token !== localHitsToken) return; // 已有更新的输入，丢弃这次结果
    localIndex = index;
  } catch {
    localIndex = null;
  }
...
async function actOnLocalPath(kind: "open" | "reveal", path: string) {
  const status = await fetchStatus();
  if (!status) {
    notify("需要本地归档服务：请运行 npm run dev:archive");
    return;
  ...
  try {
    if (kind === "open") await openEntry(path);
    else await revealEntry(path);
    notify(`${kind === "open" ? "已打开" : "已在资源管理器中定位"}：${path}`);
  } catch (error) {
...
document.addEventListener("click", (e) => {
  ...
  if (action === "local-archive") {
    el.focus({ preventScroll: true });
    if (modal) closeModal(() => localArchive.toggle());
    else localArchive.toggle();
  }
  if (action === "close-modal") closeModal();
  if (action === "bookmark") toggleSaved();
  if (action === "reveal-archive") void revealSelectedArchive();
  if (action === "exit") {
    exitSystem();
    return;
  }
  if (action === "local-reveal") {
    void actOnLocalPath("reveal", el.dataset.path!);
    return;
  }
  if (action === "local-open") {
    void actOnLocalPath("open", el.dataset.path!);
    return;
  }
  if (action === "local-more") {
    const query = el.dataset.query ?? searchQuery;
    closeModal(() => localArchive.openWithQuery(query));
    return;
  }
...
function bootFrame(t: number) {
  ...
  if (t >= 35) {
    // 原片时间轴结束。网页版直接进入浏览（阵列），由用户自己选取档案——
    // 自动展开头一张盘会让人以为"档案是替你选好的"。
    // 壁纸版仍按「入场详情」属性决定是否自动展开选中的档案。
    const showDetail =
      isWallpaper &&
      frozenTime === null &&
      openingShowsDetail(
        wallpaperHost()?.properties.openingdetail?.value,
        !!workbench?.enabled,
      );
    setMode(showDetail ? "detail" : "archive");
    return undefined;
  }
...
async function start() {
  try {
    ...
    ready = true;
    select(0);
    // 自动开场：logo 先显示 1 秒，再渐出，然后直接播放，不等点击。
    await beginOpening();
  } catch (error) {
    console.error(error);
    $("#loading").innerHTML =
...
/**
 * 开场入口序列：先尝试解锁音频，再让 logo 停留 1 秒，然后渐出并开始播放。
 * 无手势时浏览器通常不允许启动音频设备（unlock 会失败），此时本次开场静音，
 * 但用户的音效偏好保持不变，首次真实交互会自动把声音接上。
 */
async function beginOpening() {
  const wantsSound = !isWallpaper && !reviewEntry && (prefs.sound || prefs.music);
  let audible = true;
  if (isWallpaper) {
    ...
    audio.holdForEntry();
    if (prefs.music) void audio.prepareMusic().catch(() => { /* 首次交互时会补上 */ });
    audible = await Promise.race([
      audio.unlock().catch(() => false),
      new Promise<boolean>(resolve => { setTimeout(() => resolve(false), ENTRY_UNLOCK_TIMEOUT); }),
    ]);
  }
  // 逐帧对照（?time= / ?review=1）不加这段停留，避免影响对照工具。
  await new Promise(resolve => setTimeout(resolve, reviewEntry ? 0 : ENTRY_LOGO_HOLD));
  if (!audible) armAudioUnlock();
  completeStartup();
}

// ---------- 退出系统 ----------
// 白屏 → 标志快闪两下 → 下方出现 SHUTDOWN → 关窗。
// 时序由 CSS 动画的 delay 驱动（见 style.css 的 .shutdown），
// JS 只负责显示、并在最后一步关窗。
const SHUTDOWN_CLOSE = 880;
let exiting = false;

function exitSystem() {
  if (exiting || !started || !ready) return;
  exiting = true;
  $("#stage").dataset.shutdown = "true";
  audio.play("page-close");
  const overlay = $("#shutdown");
  overlay.hidden = false;
  requestAnimationFrame(() => overlay.classList.add("visible"));
  window.setTimeout(closeWindow, SHUTDOWN_CLOSE);
}

/** 关窗。浏览器只允许脚本自己打开的窗口关闭，关不掉时退化为空白页，
 *  免得停在一个"看起来已退出、其实还开着"的中间状态。 */
...
/** 首次真实交互时补解锁音频，让音乐与后续交互音效接上。一次性。 */function armAudioUnlock() {
...
function completeStartup() {
  ...
  loading.inert = true;
  setTimeout(() => {
    // 自动开场没有用户手势，不再抢焦点——右上角的圆形跳过按钮保持"不起眼"。
    loading.remove();
  }, fade);
  requestAnimationFrame(frame);
...
// 本地归档面板：检索 E:\归档 全量索引，并在本地服务在线时执行归档写操作。
localArchive = new LocalArchivePanel($("#stage"), notify);
void start();// Deterministic review controls: the running application, never a video surrogate.
