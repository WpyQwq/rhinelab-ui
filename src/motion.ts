...
export function archiveWave(row: number, lane: number, time: number, laneOrigin = 2) {
  const t = time - 22;
  const phase = row + (lane - laneOrigin) * 0.65;
...
export function cinematicField(
  ...
  return (
    archiveWave(row, lane, time, laneOrigin) * (1 - handoff) +
